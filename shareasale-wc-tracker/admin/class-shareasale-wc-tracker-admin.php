...
class ShareASale_WC_Tracker_Admin {
	...
	*/
	...
	public function admin_init() {
        ...
        //Pre-populate the AWIN ID field.
        if ($_SERVER['QUERY_STRING'] == 'page=shareasale_wc_tracker&settings-updated=true') {
            if (!empty($options['merchant-id'])) {
                $shareasale_api = new ShareASale_WC_Tracker_API( $options['merchant-id'], $options['api-token'], $options['api-secret'] );
                $shareasale_api->master_tag_id()->exec();
                $master_tag_id_response = $shareasale_api->get_response();

                if (is_numeric($master_tag_id_response)) {
                    if ($mastertag['id'] != $master_tag_id_response) {
                        $mastertag['id'] = $master_tag_id_response;
                        update_option( 'shareasale_wc_tracker_mastertag', $mastertag );
                    }
                }
                else {
                    $masterTagIdText = ' Please contact ShareASale customer support to get your AWIN ID.';
                }
            }
        }
        add_settings_field( 'awin-id', 'AWIN ID', array( $this, 'render_settings_input' ), 'shareasale_wc_tracker', 'shareasale_wc_tracker_required',
			array(
				...
				//mimicking WordPress's own " $type='$type'" output format for disabled(), checked(), and selected() functions...
				'status'      => "autocomplete='off' pattern=\d* maxlength=6 readonly",
				'size'        => 22,
				'type'        => 'text',
				'placeholder' => 'AWIN ID',
				'class'       => 'shareasale-wc-tracker-option',
                'append_text' => $masterTagIdText
			)
		);
